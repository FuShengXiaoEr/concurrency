...
public class SingleExample4 {
    ...
    private static SingleExample4 intance = null;
    ...
    public static SingleExample4 getIntance(){
        // 双重检测机制
        if (intance == null) { // 线程B 此时B看到A有内存地址了，不为null，就直接返回实例，此时调用时就会报错，提示对象未创建
            synchronized (SingleExample4.class) {
                // 同步锁
                if (intance == null) {
                    intance = new SingleExample4();  // 线程A 当线程A在执行优化后的创建实例指令时，先走到三步，此时已经拿到内存地址了，但是还没有创建实例
                }
            }
        }
        return intance;
